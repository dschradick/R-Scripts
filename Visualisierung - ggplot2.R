...
#### Diveres
## Jupyter notebook Plot größe
# => besonders gut bei horizontalen boxplots 
options(repr.plot.width=8, repr.plot.height=4)
## Beschrifungen
labs(legend="Überschrift Legende", caption="Dies ist die Caption")


#### Elemente der Grammatik
# => sind jeweils jeweils Layers
# 1. Layer: Data = Dataset (z.B. Iris)
...
# 6. Layer: Cooridinates: spezifiziert die Dimensionen des Plots
# 7. Layer: Themes: Ink, welche nicht mit den Daten zu tun hat
...
mtcars %>%            
  group_by(cyl) %>% 
  n() %>% 
  ggplot(aes(cyl,weight=n)) + geom_bar()
...
ggplot(mtcars, aes(mpg,fill=cyl)) + geom_histogram()
...
# z.B. z.B. geom_histogramm verwendet stat_bin = Anzal der Elemente in den jeweiligen Gruppe bestimmen
# geom_bar verwndet stat_bin als default stat
